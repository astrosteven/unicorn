...
export default function MapViewer({
  ...
  // Load this field's allowed-PA lookup (generated by scripts/make_nirspec_v3pa.py). Missing
  // JSON is fine — the panel simply shows no achievability flag for that field.
  useEffect(() => {
    let alive = true;
    setV3paData(null);
    const id = field.field.toLowerCase();   // "CEERS" -> "ceers", "GOODS-S" -> "goods-s"
    fetch(`/unicorn/nirspec/v3pa/${id}.json`)
      .then(r => (r.ok ? r.json() : null))
      .then(d => { if (alive && d && Array.isArray(d.samples)) setV3paData(d as V3paData); })
      .catch(() => { /* no lookup for this field — panel behaves as before */ });
    return () => { alive = false; };
  }, [field.field]);
