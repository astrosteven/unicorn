...
export const SEARCH_FIELDS: { field: string; dir: string; prefix: string; version: string; available: boolean }[] = [
  ...
  { field: "AS1063",        dir: "AS1063",        prefix: "as1063",       version: "0.98", available: false },
];
...
export async function loadField(fc: FieldConfig): Promise<{ idx: FieldIndex; zg: ZGrid }> {
  ...
  _indexPromise[fc.field] = (async () => {
    ...
    try {
      ...
      if (insp && idx.id) {
        const ids = idx.id;
        if (insp.removed.size && idx.selected) {
          const sel = idx.selected;
          for (let i = 0; i < ids.length; i++) if (insp.removed.has(ids[i])) sel[i] = 0;
        }
        if (insp.inspected.size && idx.inspected) {
          const ins = idx.inspected;
          for (let i = 0; i < ids.length; i++) if (insp.inspected.has(ids[i])) ins[i] = 1;
        }
      }
    } catch { /* override missing/broken: serve the index as published */ }
...
// Per-field inspection override: the ids the visual inspection removed from ("remove") /
// kept in ("keep") the selected sample, plus every id with ANY decision (→ inspected=1).
// Two sources, MERGED: (1) the STATIC projection <prefix>_inspect_v<ver>.json (make_inspect_
// projection.py from Supabase — the baked, everyone-sees-it view) and (2) the LIVE Supabase
// `inspections` table read with the logged-in client, so a decision reflects IMMEDIATELY in
// search / map / ★ badge without waiting for the projection to re-bake. loadField applies
// `removed`→selected=0 and `inspected`→inspected=1. Absent + not-signed-in = null.
type InspectOverride = { removed: Set<number>; kept: Set<number>; inspected: Set<number> };
const _inspectCache: Record<string, InspectOverride | null> = {};
export async function loadInspect(fc: FieldConfig): Promise<InspectOverride | null> {
  ...
  const res: InspectOverride | null =
    (data || removed.size || kept.size || inspected.size) ? { removed, kept, inspected } : null;
  _inspectCache[fc.field] = res;
  return res;
}

// Drop the cached index + inspect override for all fields so the next loadField re-reads the
// LIVE inspections. Called when leaving the inspector, so a just-made decision shows up in a
// subsequent search/map in the same session (the module index cache would otherwise be stale).
export function clearInspectCaches(): void {
  for (const k of Object.keys(_inspectCache)) delete _inspectCache[k];
