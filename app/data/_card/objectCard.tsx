...
export async function loadField(fc: FieldConfig): Promise<{ idx: FieldIndex; zg: ZGrid }> {
  ...
  _indexPromise[fc.field] = (async () => {
    ...
    try {
      ...
      if (insp && insp.removed.size && idx.selected && idx.id) {
        const ids = idx.id, sel = idx.selected;
...
// Per-field inspection override (<prefix>_inspect_v<ver>.json): the ids the visual
// inspection removed from ("remove") — and kept in ("keep") — the selected sample.
// Generated by scripts/make_inspect_projection.py from the Supabase `inspections` table.
// loadField applies `removed` → selected=0 at load. Absent (no inspection yet) = null.
const _inspectCache: Record<string, { removed: Set<number>; kept: Set<number> } | null> = {};
export async function loadInspect(fc: FieldConfig): Promise<{ removed: Set<number>; kept: Set<number> } | null> {
  ...
  const res = data
    ? { removed: new Set<number>((data.removed ?? []).map(Number)), kept: new Set<number>((data.kept ?? []).map(Number)) }
    : null;
